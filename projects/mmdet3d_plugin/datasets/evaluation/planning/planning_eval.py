...
class PlanningMetric():
    ...
    def reset(self):
        ...
        self.L2_R = torch.zeros(self.n_future)
        self.L2_L = torch.zeros(self.n_future)
        self.L2_S = torch.zeros(self.n_future)
        self.total_R = torch.tensor(0)
        self.total_L = torch.tensor(0)
        self.total_S = torch.tensor(0)
    ...
    def update(self, trajs, gt_trajs, gt_trajs_mask, fut_boxes, command):
        ...
        if command == 0:
            self.L2_R += L2.sum(dim=0)
            self.total_R += len(trajs)
        elif command == 1:
            self.L2_L += L2.sum(dim=0)
            self.total_L += len(trajs)  
        elif command == 2:
            self.L2_S += L2.sum(dim=0)
            self.total_S += len(trajs)                        
    ...
    def compute(self):
        return {
            ...
            'L2_R': self.L2_R / self.total_R,
            'L2_L': self.L2_L / self.total_L,
            'L2_S': self.L2_S / self.total_S,
        }
...
def planning_eval(results, eval_config, logger):
    ...
    for i, data in enumerate(tqdm(dataloader)):
        ...
        planning_metrics.update(pred_sdc_traj[:, :6, :2], sdc_planning[0,:, :6, :2], sdc_planning_mask[0,:, :6, :2], fut_boxes, command)
