        ...
        for gt_idx, gt_box in enumerate(gt_boxes[pred_box.sample_token]):
            ...
            if gt_box.detection_name == class_name and not (pred_box.sample_token, gt_idx) in taken:
                ...
                if this_distance < min_dist:
                    min_dist = this_distance
                    match_gt_idx = gt_idx
                    fde_distance = traj_fde(gt_box, pred_box, final_step=pred_box.traj.shape[1])
...
def prediction_metrics(gt_box_match, pred_box, miss_thresh=2):
    ...
    pred_step = pred_traj_valid.shape[1]
    dist = np.linalg.norm(pred_traj_valid - gt_traj[np.newaxis][:, :pred_step, :], axis=2)
