...
class VeteranDriverDataset(Dataset):
    DefaultAttribute = {
        "car": "vehicle.parked",
    ...
    def __init__(
        ...
        if dataset_resample:
            self.balanced_infos_resampling()
    ...
    def analyse_data_distribution(self, infos):
        """
        Split the dataset according to the command
        """
    ...
    def balanced_infos_resampling(self):
        """
        Resampling of datasets according to commands
        """
        resampling_info = []
        ...
        # resampling for turn left and right
    ...
    def load_annotations(self, ann_file):
        ...
        if 'flags' in data:
            self.flag = data['flags'][:: self.load_interval]
        else:
    ...
    def get_ann_info(self, index):
        ...
        if 'gt_agent_fut_trajs' in info:
            anns_results['gt_agent_fut_trajs'] = info['gt_agent_fut_trajs'][mask].astype(np.float32)
            anns_results['gt_agent_fut_masks'] = info['gt_agent_fut_masks'][mask].astype(np.float32)

        if 'gt_ego_fut_trajs' in info:
            anns_results['gt_ego_fut_trajs'] = info['gt_ego_fut_trajs'].astype(np.float32)
            anns_results['gt_ego_fut_masks'] = info['gt_ego_fut_masks'].astype(np.float32)
            ...
            if self.version != 'v1.0-trainval':
                ...
                for i in range(1, fut_ts + 1):
                    ...
                    try:
                        ...
                        break
                    fut_flag = self.flag[index + i]
                    if cur_flag != fut_flag:
                        fut_boxes.append(info["gt_boxes"])
                        break
                    if self.use_valid_flag:
            ...
            for i, box in enumerate(boxes):
                ...
                if np.sqrt(box.velocity[0] ** 2 + box.velocity[1] ** 2) > 0.2:
                    if name in [
                        ...
                        attr = NuScenes3DDatasetLight.DefaultAttribute[name]
                else:
