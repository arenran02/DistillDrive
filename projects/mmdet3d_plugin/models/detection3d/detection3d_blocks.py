...
@POSITIONAL_ENCODING.register_module()
class SparseBox3DEncoderLight(BaseModule):
    def __init__(
        self,
        embed_dims,
        vel_dims=3,
    ...
    def forward(self, box_3d: torch.Tensor):
        pos_feat = self.pos_fc(box_3d[..., [X, Y, Z]])
        size_feat = self.size_fc(box_3d[..., [W, L, H]])
        cos_yaw = torch.cos(box_3d[..., [YAW]])
        sin_yaw = torch.sin(box_3d[..., [YAW]])
        yaw_feat = self.yaw_fc(torch.cat([cos_yaw, sin_yaw], dim=-1))
        if self.mode == "add":
            output = pos_feat + size_feat + yaw_feat
        elif self.mode == "cat":
            output = torch.cat([pos_feat, size_feat, yaw_feat], dim=-1)
        ...
        if self.vel_dims > 0:
            vel_feat = self.vel_fc(box_3d[..., VX - 1 : VX -1 + self.vel_dims])
            if self.mode == "add":
                output = output + vel_feat
            elif self.mode == "cat":
                output = torch.cat([output, vel_feat], dim=-1)
