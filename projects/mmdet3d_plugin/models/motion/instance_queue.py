...
class InstanceQueue(nn.Module):
    ...
    def get(
        ...
        ego_feature, ego_anchor = self.prepare_planning(plan_mode_query, mask)
        # temporal stacking
        ...
        period = torch.cat([self.period, ego_period], dim=1)
    ...
    def prepare_planning(
        ...
        # ego instance init
        batch_size = plan_mode_query.shape[0]
        ...
        ego_anchor = ego_anchor.repeat(1, ego_feature.shape[1], 1) # [B, M, 11]
        # ego period update
    ...
    def cache_motion(self, instance_feature, det_output, metas):
        ...
        instance_id = det_output['instance_id']
        self.metas = metas
        self.prev_confidence = det_confidence.detach() # [B, N]
