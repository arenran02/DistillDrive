...
class SparseBox3DMotionDecoder(SparseBox3DDecoder):
    ...
    def decode(
        ...
        if squeeze_cls:
            cls_scores, cls_ids = cls_scores.max(dim=-1) # [B, N]
            cls_scores = cls_scores.unsqueeze(dim=-1) # [B, N, 1]

        box_preds = box_preds[output_idx] # [B, N, 11]
...
class HierarchicalPlanningDecoder(object):
    ...
    def decode(
        ...
        classification = planning_output['classification'][-1] # [B, 1, M]
        prediction = planning_output['prediction'][-1] # [B, 1, M, T, 2]
        bs = classification.shape[0] # batch size
        classification = classification.reshape(bs, 3, self.ego_fut_mode) # [B, C, M]
        prediction = prediction.reshape(bs, 3, self.ego_fut_mode, self.ego_fut_ts, 2).cumsum(dim=-2) # [B, C, M, T, 2]
