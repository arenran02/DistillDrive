...
from tools.visualization_dlp.bev_render import BEVRender
...
class Visualizer:
    def __init__(
        ...
        self.dlp_gt_dir = os.path.join(self.out_dir, 'bev_gt')
    ...
    def add_vis(self, index):
        ...
        self.combine(bev_gt_path, bev_pred_path, index)
    
    def combine(self, bev_gt_path, bev_pred_path, index):
        bev_gt = cv2.imread(bev_gt_path)
        bev_image = cv2.imread(bev_pred_path)
        merge_image = cv2.hconcat([bev_image, bev_gt])
