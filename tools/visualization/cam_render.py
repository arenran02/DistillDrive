...
class CamRender:
    def __init__(
        ...
        ann_file_client_args = file_client_args

        if ann_file_client_args['backend'] == 'petrel':
            self.file_client = mmcv.FileClient(**ann_file_client_args)
        else:
    ...
    def load_image(self, data_path, cam):
        ...
        if self.file_client is None:
            ...
            name = 'aoss_zxh:s3://public-dataset' + data_path.split('data')[1]
            with self.file_client.get_local_path(name) as local_path:
                image = mmcv.imread(local_path)
